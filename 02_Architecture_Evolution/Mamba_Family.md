...
Mamba-3（《Improved Sequence Modeling using State Space Principles》，ICLR 2026，Princeton PLI / Tri Dao 团队）延续 Mamba-2 的 SSD 框架，从"状态空间原理"出发做了三项核心改进。整体定位被概括为"为推理时代打造的 SSM"（The First SSM Built for the Inference Age）——即在保持训练效率的同时，重点提升解码（decode）阶段的表达力和硬件利用率。

**1. 更精确的离散化：梯形法则（Trapezoidal Discretization）**

前代 Mamba 用的是欧拉（Euler）一阶离散化，把连续状态方程近似到离散递推时误差较大。Mamba-3 改用**梯形法则**这一更高阶的数值积分近似——直观上相当于同时参考区间两端而非只看一端，离散化更贴近真实的连续动力学，在不增加多少计算量的前提下提升了建模精度。

**2. 复数 / 旋转状态（Complex & Rotational State）**

Mamba-2 的状态转移本质是实数标量衰减，只能表达"逐渐遗忘"这类单调动态。Mamba-3 引入**复数值 / 旋转形式的状态更新**，让隐状态能够表达周期性、振荡性的模式（类似 RoPE 用旋转编码相对位置的思路）。这扩展了 SSM 的表达能力，尤其增强了对需要计数、周期结构、状态跟踪等任务的建模能力——这类任务恰是纯 SSM 过去的短板。

**3. MIMO 化以提升解码吞吐（Multi-Input Multi-Output）**

前代 SSM 在解码时是逐 token 的，算术强度（arithmetic intensity）低，GPU 利用率不高——本质上是 memory-bound。Mamba-3 把 SSM 从单输入单输出（SISO）推广为**多输入多输出（MIMO）**结构，让状态更新能以更大的矩阵运算形式进行，提升了 tensor core 利用率和解码阶段的吞吐，而不增加状态大小。这正是它"为推理而生"的关键所在。

> 小结：Mamba-2 用 SSD 理论打通了"SSM ≈ 注意力"的训练效率，Mamba-3 则在此之上，通过更好的离散化、更强的状态表达（复数旋转）、更高的硬件利用率（MIMO），把重心推向了推理阶段的性能。
...
- [Mamba-3: Improved Sequence Modeling using State Space Principles (ICLR 2026)](https://openreview.net/forum?id=HwCvaJOiCj)
