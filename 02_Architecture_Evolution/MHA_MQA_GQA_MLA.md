...
LLM在推理时，每生成一个新 token，都需要让当前 token 的 Q 去访问历史 token 的 K 和 V。为了避免每一步都重新计算历史 token 的 K/V，推理系统会保存 **KV Cache**。

不同注意力机制的主要差异在于如何平衡以下两者的权衡：

> **多头注意力模型的表征能力与推理阶段 KV Cache 的显存占用及内存带宽消耗。**

其中：

- **MHA**：每个 Query Head 都有独立的 K/V Head，表达能力强，但 KV Cache 最大。
- **MQA**：所有 Query Head 共享一组 K/V Head，KV Cache 最小，但可能损失表达能力。
...
值得注意的是：并不是每个 head 都一定学到完全不同的功能。实际模型中可能存在冗余 head，这也是后来 MQA、GQA、Head Pruning 等方法能够成立的原因之一。
...
MQA 的代价是 K/V 表示能力变弱。因为所有 Query Head 只能面对同一份 K/V 表示，模型可用的注意力上下文子空间减少，性能可能下降，尤其是在大模型和复杂任务上更明显。
...
| MQA | 多个 Q Head 共享一组 K/V，缓存小，可能掉性能  |
| GQA | 一组 Q Head 共享一组 K/V，兼顾性能和效率    |
...
MHA 提供了多头表示能力，但 KV Cache 随 head 数线性增长。MQA 通过共享单组 K/V 极大减少缓存和内存带宽，但可能牺牲表达能力。GQA 通过设置中间数量的 K/V Head，在性能和效率之间取得折中。MLA 则进一步改变缓存对象，把完整 K/V 替换为压缩 latent 和解耦位置向量，是面向长上下文和高吞吐推理的进一步优化。
...
- [Transformer / MHA](https://arxiv.org/abs/1706.03762)
